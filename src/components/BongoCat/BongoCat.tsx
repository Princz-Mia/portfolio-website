...
import "./bongo-cat.scss";

// Inspired By Caroline Artz
// https://codepen.io/carolineartz/pen/qBOEzQa
const BongoCat = () => {
  const [isEyesClosed, setIsEyesClosed] = useState(false);
  const [clickCount, setClickCount] = useState(0);
  const [isAngry, setIsAngry] = useState(false);
  const terminalTlRef = useRef<gsap.core.Timeline | null>(null);
  const pawTlRef = useRef<gsap.core.Timeline | null>(null);
  const noteTlRef = useRef<gsap.core.Timeline[] | null>(null);

  const handleHeadClick = () => {
    if (isAngry) return; // ha már mérges, nem csinálunk semmit
    setClickCount((prev) => {
      const newCount = prev + 1;
      if (newCount >= 5) {
        setIsAngry(true);
        if (terminalTlRef.current) {
          terminalTlRef.current.kill();
        }
        if (pawTlRef.current) {
          pawTlRef.current.kill();
        }
        if (noteTlRef.current) {
          noteTlRef.current.forEach((tl) => tl.kill());
          noteTlRef.current = null;
        }
        setTimeout(() => {
          setIsAngry(false);
          setClickCount(0);
    ...
    // Csak normál állapotban zárja be a szemet és rángatja a fejet
    setIsEyesClosed(true);
    shakeAnimation();
    setTimeout(() => setIsEyesClosed(false), 500);
  };

  const shakeAnimation = () => {
    const tl = gsap.timeline();
    const shakeSelectors =
      ".head, .headphone-left .speaker, .headphone-right .speaker";
    tl.to(shakeSelectors, {
      duration: 0.05,
      rotation: -3.5,
  ...
  useEffect(() => {
    ...
    // Csak normál állapotban fut le a mancsa animáció
    if (!isAngry) {
      const tl = gsap.timeline();
    ...
    const noteEls = noteElFn(document.querySelectorAll(music.note));
    ...
    const timelineArray = [
      ...
      animateNotes(notesG2),
      animateNotes(notesG3),
    ];
    noteTlRef.current = timelineArray;

    if (!isAngry) {
      const terminalLines = document.querySelectorAll(".terminal-code line");
      const terminalTl = gsap.timeline({ repeat: -1, repeatDelay: 0.5 });
      terminalTlRef.current = terminalTl;
      terminalLines.forEach((line) => {
        ...
        terminalTl.to(line, { autoAlpha: 1, duration: 0 }).to(line, {
          strokeDashoffset: 0,
          duration: 0.5,
          ease: "power1.inOut",
        });
  ...
  useEffect(() => {
    if (isAngry) {
      const angryEffect = document.querySelector(".angry-effect");
      if (angryEffect) {
        gsap.to(angryEffect, {
          scale: 1.2,
          duration: 0.3,
  ...
  useEffect(() => {
    ...
    notes.forEach((note) => {
      if (!note.classList.contains("cloned")) {
        note.classList.add("cloned");
  ...
  return (
    <div className="relative w-full h-full max-w-screen mx-auto p-4">
      <svg
      ...
      >
        <g id="bongo-cat">
          ...
          >
            ...
            {isAngry ? (
              <>
                {/* Megújított angry eyebrows – a belső végek lejjebb, így szorosabban illeszkednek a szemekhez */}
                <g className="angry-eyebrows">
                  <path
                    d="M355,145 L395,160"
                    stroke="(--orange)"
                    strokeWidth="4"
                  ...
                  <path
                    ...
                    strokeWidth="4"
                    strokeLinecap="round"
                    transform="rotate(-100,492,148)"
                  />
                </g>
                {/* Angry eyes */}
                <path d="M375.4,168.9c0,.3-.8,3.6,1.5,6a5.9,5.9,0,0,0,7.2,1.4,6.1,6.1,0,0,0,2.2-7.7c-1.5-3.1-5.7-4.5-7.3-3.2s-.8,6,1,6.6,3.3-.7,3.3-2.1-1.5-1.8-1.6-1.9"></path>
                <path d="M485,179.2a6.6,6.6,0,0,0-4.9,3.6,6,6,0,0,0,1.5,7.3,6,6,0,0,0,7.9-1c2.3-2.6,2-7,.2-8s-5.9,1.6-5.7,3.5,1.9,2.8,3.2,2.3,1.1-2.2,1.1-2.3"></path>
              </>
            ) : isEyesClosed && clickCount < 3 ? (
              <>
                <path
                  d="M360,172c5-3,10-3,15,0"
                  transform="rotate(5 370,172)"
                  strokeWidth="5"
                  fill="none"
                  strokeLinecap="round"
                />
                <path
                  d="M470,187c5-3,10-3,15,0"
                  transform="rotate(8 480,182)"
                  strokeWidth="5"
                  fill="none"
                  strokeLinecap="round"
                />
            ...
            <g className="headphone headphone-right">
              ...
              <path
                className="band band-right"
                d="M527,43.6c-15.9-4.6-57-14.1-104,2.3a166.9,166.9,0,0,0-60.9,37.3"
              ></path>
            </g>
          </g>
          <g className="music music-right">
            ...
            <g className="note">
              <g>
                ...
                <path d="M350,82.3c0-3.1-4.5-5.7-8.2-5.9s-9.3,2.8-9.2,6,4.7,5.7,8.6,5.7Z"></path>
                <path d="M382.7,89.9c0-3.1-4.4-5.7-8.2-5.8s-9.3,2.7-9.2,5.9,4.7,5.7,8.7,5.7Z"></path>
              </g>
              <g>
                <polyline points="350 81.7 350 43.5 382.7 50.7 382.7 89.5"></polyline>
                <path d="M350,82.3c0-3.1-4.5-5.7-8.2-5.9s-9.3,2.8-9.2,6,4.7,5.7,8.6,5.7Z"></path>
            ...
            <g className="note">
              ...
              <path d="M388.2,74.1c0-3-4.4-5.6-8.1-5.8s-9.2,2.8-9.1,6,4.6,5.6,8.6,5.6Z"></path>
              <path d="M354.9,81.9c0-3.1-4.4-5.7-8.2-5.9s-9.3,2.8-9.2,6,4.7,5.7,8.7,5.7Z"></path>
              <line x1="354.9" y1="48.4" x2="388.2" y2="40.3"></line>
              <line x1="354.9" y1="54.6" x2="388.2" y2="47"></line>
            </g>
            <g className="note">
              <g>
                <path d="M371.8,79.5c0-3.1-4.5-5.8-8.3-5.9s-9.3,2.8-9.2,6,4.7,5.7,8.7,5.7Z"></path>
          ...
          {/* Jobb mancsa: ha mérges, csak a "down" állapot jelenik meg */}
          {isAngry ? (
            <g className="paw paw-right">
              <path
                className="down"
                d="M289.1,181.7c-12.1,9.8-20.6,20.7-20.7,32.1-0.2,9,3.8,20.4,13.3,25.2s20.1,0.6,29.6-3.4c13.4-5.7,23.9-14.6,29.4-21.5"
              />
            </g>
          ) : (
            <g className="paw paw-right">
              ...
              />
              <g className="up">
                <path d="M327.3,170c-0.4-1.4-6.3-18.8-23.5-23.5-0.8-0.2-18.6-4.7-28.9,6.3-8.4,9.1-6,22.5-4.6,30.2a54.3,54.3,0,0,0,8.1,19.9" />
                <g className="pads">
                  <path d="M297.2,154.8c1-0.5,2.7-0.1,3,0.6s-1.4,2.4-2.6,2.1a1.6,1.6,0,0,1-1.1-1.2A1.6,1.6,0,0,1,297.2,154.8Z" />
                  <path d="M285.8,159.4c0.3-0.4,1-1.1,1.7-0.8s0.9,1.4,0.8,2.2-1.8,2.1-2.5,1.5S285.2,160.4,285.8,159.4Z" />
                  <path d="M276.9,171c0.5-0.4,2.7-0.3,3.2,0.6s-0.6,1.8-1.4,1.8S276.2,171.6,276.9,171Z" />
                  <path d="M296.4,168.6c2.3-0.9,6.4,6.3,7.6,9s-5.2,4.5-7.4,6-5.1-6.1-5.9-8.3S293.7,169.8,296.4,168.6Z" />
                </g>
              </g>
            </g>
          )}
          ...
          ></polygon>
          {/* Bal mancsa: szintén, mérges esetben csak a down állapot */}
          {isAngry ? (
          ...
          <g className="music music-left">
            ...
            <g className="note">
              <g>
                ...
                <path d="M614.8,155.7c0-3.1-4.4-5.7-8.2-5.9s-9.2,2.8-9.2,6,4.7,5.6,8.7,5.6Z"></path>
                <path d="M647.5,163.3c.1-3.1-4.4-5.7-8.2-5.9s-9.2,2.8-9.1,6,4.7,5.7,8.6,5.7Z"></path>
              </g>
              <g>
                <polyline points="614.8 155 614.8 116.8 647.5 124 647.5 162.9"></polyline>
                <path d="M614.8,155.7c0-3.1-4.4-5.7-8.2-5.9s-9.2,2.8-9.2,6,4.7,5.6,8.7,5.6Z"></path>
            ...
            <g className="note">
              ...
              <path d="M646.5,149c0-3.1-4.4-5.7-8.1-5.8s-9.2,2.7-9.1,5.9,4.7,5.6,8.6,5.6Z"></path>
              <path d="M613.2,156.7c.1-3.1-4.4-5.7-8.2-5.8s-9.3,2.7-9.2,6,4.7,5.6,8.7,5.6Z"></path>
              <line x1="613.2" y1="123.2" x2="646.5" y2="115.1"></line>
              <line x1="613.2" y1="129.4" x2="646.5" y2="121.8"></line>
            </g>
            <g className="note">
              <g>
                <path d="M636.6,152.9c0-3.2-4.4-5.8-8.2-5.9s-9.3,2.8-9.3,6,4.8,5.7,8.7,5.7Z"></path>
